...
mdd.pheno <- unfiltered.mostafavi[, 'class']
# 463 MDD, 452 HC
...
fam.data <- read_table('MDD_LD.fam', col_names = 
    c('pedigree', 'member', 'father', 'mother', 'sex', 'affected')) %>% 
  mutate(short.ids = strsplit(pedigree,'_')[3])


# sex.column <- fam.data$sex
# 
# gwas.subj.ids <- sapply(1:length(gwas.LONG.ids), splitFN, 
#                         string_vector = gwas.subject.LONG.ids)
...
chisq.test(mdd.pheno,sex.vec)
table(mdd.pheno,sex.vec)
...
filtered.mostafavi.df <- data.frame(filter$fdata, class = mdd.pheno)
