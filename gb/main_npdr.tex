...
\abstract{
{\bf Summary:} Machine learning feature selection methods are needed to detect complex interaction-network effects in complicated modeling scenarios in high-dimensional data, such as GWAS, gene expression, eQTL, and structural/functional neuroimage studies for case-control or continuous outcomes.
...
{\bf Availability: https://github.com/insilico/npdr} 
}
...
Thus, computationally scalable feature selection methods are needed to capture these higher-order effects in high-dimensional data, such as genome-wide association studies (GWAS)~\citep{titv} and RNA-Seq studies~\citep{stir}.
...
The models allow any predictor data type, such as variants in GWAS or RNA-Seq expression levels.
...
Because NPDR and other Relief-based feature selection methods are based on distances between instances, we first describe the algorithms and notation for identifying nearest neighbors in the space of all attributes.
