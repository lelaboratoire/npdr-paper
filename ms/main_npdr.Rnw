...
As demonstrated in the subsequent section, when proper filtering on the attributes is applied, the runtime will be greatly reduce.
...
Proper preprocessing of the dataset retains genes with low sex association (Supplementary Fig. S4), ameliorates the curse of dimensionality and thus leads to a fast NPDR computation of 99 seconds (on the same desktop with an Intel Xeon W-2104 CPU and 32GB of RAM).
