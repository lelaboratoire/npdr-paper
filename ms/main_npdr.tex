...
We now apply the GLM formalism to enable NPDR to handle dichotomous outcome data (e.g., case-control phenotype). The STIR method was designed for statistical testing in case-control data, but as we have seen NPDR can also handle continuous outcomes and adjust for covariates.
...
\hline
Importance Score \,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\, (Case-Control)  & $\bar{M}_a - \bar{H}_a$  & $\dfrac{\bar{M}_a - \bar{H}_a }{S_p[M,H]\sqrt{\frac{1}{|M|}+\frac{1}{|H|}}}$ &  $\beta_a$ coefficient \\
Score has a null distribution & No                               & Yes     & {\bf Yes}  \\
Supports continuous outcome   & Yes                              & No      & {\bf Yes}  \\
Supports covariates           & No                               & No      & {\bf Yes} \\
\hline
\end{tabular}
\caption{Properties of standard Relief-based methods and generalizations STIR and NPDR. The coefficient $\beta_a$ is the NPDR score for case-control (Eq.~\ref{eq:logit_nocovar}) or linear regression (Eq.~\ref{eq:lin_reg}). The quantity $S_p$ in STIR is the pooled standard deviation of the hit and miss means. Only the score for case-control Relief is shown and STIR is restricted to case-control\cite{stir}.}
...
In simulated data with both continuous and case-control outcomes, NPDR yields improved attribute estimates than standard Relief (and Random Forest) (Fig. \ref{fig:npdr_relief}). 
For simulated main effects with a continuous outcome ($m = 200$ samples and $p = 1,000$ attributes with $100$ functional), we use NPDR with a linear model (Eq.~\ref{eq:lin_reg}) and we compare with standard RReliefF from CORElearn.
...
NPDR correctly detects X out of 100 functional attributes with a Bonferroni cutoff (Fig \ref{fig:npdr_relief}A).
...
Because Relief-based methods do not have a statistical threshold, we use the auPRC for comparison, which shows the improved feature selection performance of using a distance regression-based attribute score in NPDR compared to the standard RReleif score (Fig \ref{fig:npdr_relief}B).
...
The precision-recall curve clearly demonstrates NPDR outperforms Relief-F in this particular simulated dataset (Fig \ref{fig:npdr_relief}B), and across 100 simulations of moderate effect size, NPDR yields significantly higher auPRC (P < 0.0001, see Supplementary Fig. S2). While the importance scores are correlated ($R^2 = 0.869$), any RReleif threshold (vertical line in A) will yield an equal or greater false positive rate than NDPR. 
...
For case-control outcome data with network interaction effects, we use the logistic model (Eq.~\ref{eq:logit_nocovar}) in NPDR and compare the regression coefficients with Relief-F weights in CORElearn (Fig \ref{fig:npdr_relief}C-D). We again use the fixed-$k$ neighborhood for both methods to approximate a multiSURF neighborhood. Similarly, the importance scores are correlated ($R^2 = 0.848$), but any Relief-F threshold (vertical line in C) will yield an equal or greater false positive rate than NDPR and NPDR yields significantly higher auPRC than Relief (P < 0.0001, see Supplementary Fig. S2). The auPRC of NPDR and Relief in the main effect simulations are less than interaction simulations, which is due to differences in the simulated effect size. We also compare NPDR (based on logistic regression) and STIR (based on a t-test) for the case-control outcome data with interaction effects.
...
While logistic regression and the t-test have slightly different assumptions on the distribution of samples, NPDR and STIR yield highly correlated scores for case-control data with interaction effects, where $R^2$ ranges from 0.9827 to 0.9994 in 100 replications (Supplementary Fig. S1). 

\begin{figure}[!tbp]
\centerline{\includegraphics[trim = 0 0 0 0]{../figs/fig1.pdf}}
\caption{{\bf Comparison of NPDR and Relief-based importance scores} for continuous outcome data with main effects (top row) and case-control outcome data with interaction effects (bottom row). Results for one replicate simulation ($m = 200$ samples and $p = 1,000$ attributes with 100 functional). For continuous outcome (A), importance scores computed by RRelief weight and NPDR standardized linear regression coefficient. For case-control outcome (C), scores computed by Relief-F and NPDR standardized logistic regression coefficient. A regression line between the scores with $R^2$ is shown, and a 0.05 Bonferroni cutoff (dashed) is shown for NPDR (A and C). There is no statistical threshold for Relief-based methods, so area under the precision-recall curve (AUC) is used to compare algorithm performance (B, D).}
...
\hline
\end{tabular}
\caption{Genes associated with Pittsburgh Sleep Quality Index (PSQI) using NPDR analysis.}
\label{tab:psqi}
\end{table}
