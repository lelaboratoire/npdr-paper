...
load('../results/0_mdd_results.Rdata')
load('../results/rf_0_mdd_results.Rdata')
```
...
pboth <- ggplot(npdr_w_wo_sex, aes(x = nlog10.nosex, y = nlog10.sex)) + 
  ...
  coord_fixed(ratio = 1) 
pboth
ggsave(pboth, filename = '../figs/npdrs_mdd.pdf', width = 6, height = 4.5)
```
...
write_csv(sex_genes, '../results/sex_genes.csv')
write_csv(best_genes, '../results/best_0.05_npdr_genes.csv')
```
