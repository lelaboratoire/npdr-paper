...
p <- ggplot(test.df, aes(x = rrelief, y = beta.Z.att)) + 
  ...
  annotate("text", x = 0.08, y = 35, label = deparse(bquote(R^2 == .(r2))), parse = T) +
...
q <- prr.dat %>%
  ...
  annotate('text', x = 0.75, y = 0.9 , label = 'NPDR \n AUC = 0.611', color = cbbPalette[6]) +
  annotate('text', x = 0.35, y = 0.6, label = 'Relief-F \n AUC = 0.522', color = cbbPalette[8]) +
  guides(color = FALSE) + 
...
pr <- ggplot(pr_df, aes(x = type, y = aupr, fill = type, color = type)) + 
  ...
  annotate("text", x = 1.5, y = 0.36, label = paste('p < 0.0001')) + 
