...
save(npdr_w_wo_sex, uni_w_wo_sex, file = paste0('../results/', pct, '_mostafavi_mdd_results.Rdata'))
```
...
rnaseq_sex <- filtered.mostafavi.df %>%
  ...
  merge(select(pheno.sex, ids, sex), by = 'ids') %>%
  column_to_rownames('ids')

# weights <-c(rep(0.01, ncol(rnaseq_sex) - 2), 1)
