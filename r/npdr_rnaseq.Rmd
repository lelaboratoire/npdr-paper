...
rnaseq.uni <- univarRegress(
  outcome = 'class', 
  dataset = filtered.mostafavi.df, 
  regression.type = 'glm')
head(rnaseq.uni)

# rnaseq.uni[rnaseq.uni[,3]<.05,]
# write.csv(rnaseq.uni[1:20,],'ResultsRNA/univar.mdd.csv')
# rnaseq.uni[1:40,]
```
...
rnaseq.uni.sexadjust <- univarRegress(
  outcome = 'class', dataset = filtered.mostafavi.df, 
  regression.type = 'glm', covars = pheno.sex$sex)
head(rnaseq.uni.sexadjust)
# rnaseq.uni.sexadjust %>% filter(p.adj<.05)
# rnaseq.uni.sexadjust[1:10,]
# write.csv(rnaseq.uni.sexadjust[1:20,],'ResultsRNA/univar.mdd.sexadjust.csv')
```
...
```{r}
rnaseq.sex <- univarRegress(outcome = pheno.sex$sex-1,
                            dataset = filtered.pred,
                            regression.type='glm')
# rnaseq.sex[rnaseq.sex[,3]<.05,]
# write.csv(rnaseq.sex[rnaseq.sex[,3]<.05,],'ResultsRNA/univar.sexassoc.csv')
# write.csv(rnaseq.sex[1:50,],'ResultsRNA/univar.sexassoc.csv')

```


#### Compare top 20 genes for no-adjust, sex-adjusted, and sex-associated:

```{r}
topx <- 20
univ.compare <- data.frame(NoAdjust = rownames(rnaseq.uni)[1:topx],
                           SexAdjust = rownames(rnaseq.uni.sexadjust)[1:topx],
                           SexAssoc = rownames(rnaseq.sex)[1:topx])
# colnames(univ.compare) <-c('no-adjust', 'sex-adjust','sex-assoc')
univ.compare

#cbind(rnaseq.uni[1:topx,],
#      rownames(rnaseq.uni.sexadjust)[1:topx], rnaseq.uni.sexadjust[1:topx,],
#      rownames(rnaseq.sex)[1:topx], rnaseq.sex[1:topx,])
...
npdr.mdd.rnaseq.results[1:10,]  # top 10, p-value sorted
...
npdr.mdd.atts <- npdr.mdd.rnaseq.results %>%
  filter(pval.adj < 0.05) %>%
  pull(att)
npdr.mdd.atts
# npdr.mdd.rnaseq.positives <- row.names(npdr.mdd.rnaseq.results[npdr.mdd.fdr.idx,]) # reSTIR p.adj<.05
# npdr.mdd.rnaseq.positives
# npdr.mdd.rnaseq.results[npdr.mdd.atts,]
# write.csv(npdr.mdd.rnaseq.results[npdr.mdd.fdr.idx,],'ResultsRNA/NPDR.mdd.csv')
...
sexassoc.idx <- npdr.mdd.sexassoc.rnaseq.results$pval.adj<.05
# length(sexassoc.idx)
npdr.mdd.sexassoc.results[1:20,] 
topx <- 20
stir.sex.compare <- data.frame(NoAdjust = npdr.mdd.rnaseq.results[1:topx, 'att'],
                               SexAssoc = npdr.mdd.sexassoc.results[1:topx, 'att'])
                      # rownames(npdr.mdd.sexadj.rnaseq.results)[1:topx],
                      # rownames(npdr.mdd.sexassoc.results)[1:topx])
# colnames(stir.sex.compare) <-c('no-adjust', 'sex-adjust','sex-assoc')
stir.sex.compare
```

```{r}
with_without_sex <- data.frame(rnaseq.sex) %>%
  rownames_to_column('att') %>%
  merge(npdr.mdd.rnaseq.results, by = 'att') %>%
  merge(npdr.mdd.sexassoc.results, by = 'att') %>%
  mutate(nlog10_nosex = -log10(pval.adj.x)) %>%
  mutate(nlog10_sex = -log10(pval.adj.y)) %>%
  mutate(significant = as.factor((pval.adj.x < 0.05) + (pval.adj.y < 0.05))) %>%
  mutate(sig.genes = ifelse(pval.adj.x < 0.05, att, NA)) %>%
  mutate(nlog10_sex_genes = -log10(p.adj))

save(with_without_sex, file = paste0(pct, '_mdd_results.Rdata'))
```



```{r}
# with_without_sex$sig.genes <- NA
# with_without_sex[t.stir$t.pval.adj.stir < 0.05, "sig.genes"] <- t.stir[t.stir$t.pval.adj.stir < 0.05, "attribute"] 
# t.stir$t.sig <- as.factor((t.stir$regT.padj <0.05) + (t.stir$t.pval.adj.stir <0.05))

pboth <- ggplot(with_without_sex, aes(x = nlog10_nosex, y = nlog10_sex)) + 
  geom_point(alpha = 0.7, shape = 21, size = 2.5, aes(fill = nlog10_sex_genes)) + 
  theme_bw() +
  ...
  # scale_fill_continuous(label = 'p.sex.adj') + 
  labs(x = bquote('Vanilla NPDR ('~-log[10]~'p'[adj]~')'),
       y = bquote('Sex-adjusted NPDR ('~-log[10]~'p'[adj]~')')) +
  ggrepel::geom_text_repel(aes(label = sig.genes, size = significant), #check_overlap = TRUE, 
  ...
  # scale_fill_manual(values = c("white", cbbPalette[2], "grey30")) +
...
# npdr.mdd.sexadj2.rnaseq <- npdr('class', filtered.mostafavi.df, regression.type='glm', 
#                                        attr.diff.type='numeric-abs', 
#                                        nbd.method='multisurf', nbd.metric = 'manhattan', msurf.sd.frac=0.5,
#                                        covars=sex.vec,                   # works with sex.covar.mat as well
#                                        covar.diff.type='match-mismatch', # for categorical covar like sex
#                                        #rm.attr.from.dist=c(), # length=0
#                                        #rm.attr.from.dist = rownames(npdr.mdd.sexassoc.rnaseq.results)[1:topx],
#                                        # rm.attr.from.dist = rownames(rnaseq.sex[1:40,]),
#                                        padj.method='fdr', verbose=TRUE)
# rownames(npdr.mdd.sexadj2.rnaseq[npdr.mdd.sexadj2.rnaseq$pval.adj<.05,]) 
# stir.sex.compare2 <- cbind(rownames(npdr.mdd.rnaseq.results)[1:topx], 
#                            rownames(npdr.mdd.sexassoc.rnaseq.results)[1:topx],
#                            rownames(npdr.mdd.sexadj.rnaseq.results)[1:topx],
#                            rownames(npdr.mdd.sexadj2.rnaseq)[1:topx]
#                           )
...
Try penalize regressions:

```{r}
library(glmnet)
# ######################### VECTOR MEMORY EXHAUSTED (LIMIT REACHED)
# ##### Run glmnetSTIR, penalized npdr, no covar
# glmnetSTIR.mdd.results <- npdr('class', filtered.mostafavi.df, regression.type='glmnet', attr.diff.type='numeric-abs',
...
# 
# npdr.mdd.sex1.fdr.idx <- npdr.mdd.sexadj1.rnaseq.results$pval.adj<.05
# npdr.mdd.sexadj1.rnaseq.positives <- row.names(npdr.mdd.sexadj1.rnaseq.results[npdr.mdd.sex1.fdr.idx,]) # reSTIR p.adj<.05
# npdr.mdd.sexadj1.rnaseq.positives
# npdr.mdd.sexadj1.rnaseq.results[npdr.mdd.sex1.fdr.idx,]
# # write.csv(npdr.mdd.sexadj1.rnaseq.results[npdr.mdd.sex1.fdr.idx,],'ResultsRNA/NPDR.mdd.sexadjust.csv')
# 
# ##### residualize?
