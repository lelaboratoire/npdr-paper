...
p_qtrait <- ggplot(test_df_qtrait, aes(x = rrelief, y = beta.Z.att)) + 
  ...
  labs(colour = NULL, x = "RRelief score", y = bquote('NPDR linear '~beta),
       title = NULL) +
...
q_qtrait <- prr_dat_qtrait %>%
  ...
  annotate('text', x = 0.23, y = 0.58, label = paste0('RRelief \n AUC = ', auprc_relief_qtrait), color = cbbPalette[8]) +
  guides(color = FALSE)
...
p_cc <- ggplot(test_df_cc, aes(x = rrelief, y = beta.Z.att)) + 
  ...
  geom_point(aes(colour = functional), alpha = 0.7, size = 2) +
  theme_bw() + theme(legend.position = c(0.22, 0.87), legend.margin = margin(c(1,1,1,1))) +
  labs(colour = NULL, x = "Relief-F score", y = bquote('NPDR logistic'~beta),
       title = NULL) +
  scale_color_manual(values = cbbPalette[c(1,2)],
                     labels = c('Non-functional', 'Functional')) +
...
title_c <- ggdraw() + draw_label('Dichotomous outcome data with interaction effect') 
...
# Continuous outcome:
## NPDR vs Relief-F score:
p_qtrait <- ggplot(rf_npdr_qtrait, aes(x = X.IncMSE, y = beta.Z.att)) +
  geom_smooth(method = "lm", se = F, color = "grey") +
  geom_point(aes(colour = functional), alpha = 0.7, size = 2) +
  theme_bw() + theme(legend.position = c(0.22, 0.87), legend.margin = margin(c(1,1,1,1))) +
  labs(colour = NULL, x = "Random forest permutation score", y = bquote('NPDR linear '~beta),
       title = NULL) +
  scale_color_manual(values = cbbPalette[c(1,2)],
                     labels = c('Non-functional', 'Functional')) +
  annotate("text", x = 18, y = 80, label = deparse(bquote(R^2 == .(r2_rf_qtrait))), parse = T) +
  annotate("text", 0.25, npdr.beta.cutoff.qtrait + 3, label = "NPDR cutoff", size = 4, color = cbbPalette[1]) +
  geom_hline(yintercept=npdr.beta.cutoff.qtrait, linetype="dashed", alpha = 0.5)
p_qtrait
## PR curve:
q_qtrait <- prr_dat_qtrait %>%
  ...
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2), labels = scales::percent) +
  annotate('text', x = 0.74, y = 0.9 , label = paste0('NPDR \n AUC = ', auprc_npdr_qtrait), color = cbbPalette[6]) +
  annotate('text', x = 0.18, y = 0.58, label = paste0('Random forest \n AUC = ', auprc_rf_qtrait), color = cbbPalette[3]) +
  guides(color = FALSE)
q_qtrait
# Dichotomous outcome:
## NPDR vs Relief-F score:
p_cc <- ggplot(rf_npdr_cc, aes(x = MeanDecreaseAccuracy, y = beta.Z.att)) + 
  geom_smooth(method = "lm", se = F, color = "grey") + 
  geom_point(aes(colour = functional), alpha = 0.7, size = 2) +
  theme_bw() + theme(legend.position = c(0.22, 0.87), legend.margin = margin(c(1,1,1,1))) +
  labs(colour = NULL, x = "Random forest permutation score", y = bquote('NPDR logistic'~beta),
       title = NULL) +
  scale_color_manual(values = cbbPalette[c(1,2)],
                     labels = c('Non-functional', 'Functional')) +
  annotate("text", x = 3, y = 24, label = deparse(bquote(R^2 == .(r2_rf_cc))), parse = T) +
  annotate("text", 0.062, npdr.beta.cutoff.cc + 1, label = "NPDR cutoff", size = 4, color = cbbPalette[1]) +
  geom_hline(yintercept=npdr.beta.cutoff.cc, linetype="dashed", alpha = 0.5)
p_cc
## PR curve:
q_cc <- prr_rf_cc %>%
  ...
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2), labels = scales::percent) + 
  annotate('text', x = 0.71, y = 0.91 , label = paste0('NPDR \n AUC = ', auprc_npdr_cc), color = cbbPalette[6]) +
  annotate('text', x = 0.53, y = 0.4, label = paste0('Random forest \n AUC = ', auprc_rf_cc), color = cbbPalette[3]) +
  guides(color = FALSE) #+ 
...
fig_sup<- plot_grid(qp, ccp, nrow = 2)
# ggsave(fig_sup, filename = 'figs/npdr_rf.svg', height = 7, width = 7)
ggsave(fig_sup, filename = 'figs/npdr_rf.pdf', height = 7, width = 7)
```
...
pr_df <- rbind(pr_cc_100, pr_qtrait_100) %>%
  mutate(sim = rep(c('Dichotomous outcome', 'Continuous outcome'), each = nrow(pr_qtrait_100))) %>%
  # mutate(newtype = case_when((sim == 'Continuous outcome' & type == 'Relief-F') ~ 'RRelief',
  #                            TRUE ~ as.character(type)))
  mutate(newtype = as.factor(case_when((type == 'Relief-F') ~ 'Relief',
                             TRUE ~ as.character(type))))
...
p_text <- data.frame(
  ...
  sim = c('Dichotomous outcome', 'Continuous outcome'),
  x = c(1.5, 1.5), y = c(0.56, 0.47), newtype =  c('NPDR', 'Relief')
)
...
pr_100 <- ggplot(pr_df, aes(x = newtype, y = aupr, fill = newtype, color = newtype)) + 
  ...
  scale_color_manual(values = cbbPalette[c(8,6, 5)]) +
  scale_fill_manual(values = cbbPalette[c(8,6, 5)]) +
