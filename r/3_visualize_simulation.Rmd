...
npdr_text <- data.frame(
  label = c(paste0('NPDR\n', round(auroc_npdr_cc, 3)), 
            paste0('NPDR\n', round(auroc_npdr_qtrait, 3))),
  sim = c('Dichotomous outcome', 'Continuous outcome'),
  x = c(0.84, 0.85), y = c(0.88, 0.88), type = c('NPDR', 'NPDR')
)
...
roc_df <- rbind(roc_cc_100, roc_qtrait_100) %>%
  ...
  mutate(newtype = as.factor(case_when((type %in% c('Relief-F', 'RRelief')) ~ 'Relief',
                             TRUE ~ as.character(type))))
roc_df$newtype <- factor(roc_df$newtype, levels = c('Relief', 'NPDR', 'Random forest'))

p_text <- data.frame(
  label = c('***', '***'),
  sim = c('Dichotomous outcome', 'Continuous outcome'),
  y = c(0.64, 0.64), newtype =  c('NPDR', 'Relief')
)
...
roc_100 <- ggplot(roc_df, aes(x = newtype, y = aupr, fill = newtype, color = newtype)) + 
  ...
  scale_color_manual(values = cbbPalette[c(6, 8, 3)]) +
  scale_fill_manual(values = cbbPalette[c(6, 8, 3)]) +
  geom_text(p_text, mapping = aes(x = 1.5, y = y, label = label), color = 'grey20') +
  geom_text(p_text, mapping = aes(x = 2.5, y = y, label = label), color = 'grey20') +
  # annotate("text", x = 1.5, y = 0.5, label = paste('p < 0.0001')) + 
  theme_bw() + guides(fill = F, color = F)
