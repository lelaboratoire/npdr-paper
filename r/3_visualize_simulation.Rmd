...
pr <- rbind(pr_qtrait, pr_cc) %>% 
  mutate(sim = c(rep('Continuous outcome', nrow(pr_qtrait)), 
...
ggsave(pr, filename = 'figs/fig2_pr_plots.pdf', width = 5.8, height = 3.2)
```
...
pr_text <- data.frame(
  ...
  sim = rep(c('Dichotomous outcome', 'Continuous outcome'), 3),
...
ggsave(rocs, filename = paste0('figs/roc_compare_100.pdf'), height = 6.2, width = 5)
```
