...
p <- ggplot(test.df, aes(x=stir.log10,y=npdr.log10)) + 
  ...
  labs(colour = NULL, 
       ...
       title = NULL) + 
  annotate("text", x = 130, y = 130, label = deparse(bquote(R^2 == .(r2))), parse = T) +
...
p
ggsave(p, filename = paste0('../figs/npdr_stir_p_cc_', n.variables, '.pdf'), height = 4.5, width = 4.5 )
npdr_stir_p_cc <- p
npdr_stir <- test.df
```
...
p <- ggplot(test.df, aes(x = rrelief, y = beta.Z.att)) + 
  ...
  annotate("text", x = 0.06, y = 24, label = deparse(bquote(R^2 == .(r2))), parse = T) +
...
q <- prr.dat %>%
  ...
  annotate('text', x = 0.76, y = 0.93 , label = paste0('NPDR \n AUC = ', auprc_npdr), color = cbbPalette[6]) +
  annotate('text', x = 0.75, y = 0.43, label = paste0('Relief-F \n AUC = ', auprc_relief), color = cbbPalette[8]) +
  guides(color = FALSE) + 
  labs(title = "PR curve for detecting important attributes")
q  

pq <- cowplot::plot_grid(p, q, labels = 'AUTO')
ggsave(pq, filename = paste0('../figs/npdr_relief_cc_', n.variables, '.pdf'), height = 4, width = 8.5)
npdr_relief_cc <- pq
...
roc <- ggplot(ss_df, aes(x = Sensitivity, y = Specificity, color = type)) +   
  ...
  annotate('text', x = 0.85, y = 0.9, label = paste0('NPDR \n AUC = ', auroc_npdr), color = cbbPalette[6]) +
  annotate('text', x = 0.83, y = 0.5, label = paste0('Relief-F \n AUC = ', auroc_relief), color = cbbPalette[8]) +
  guides(color = FALSE) + 
  labs(title = "ROC curve for detecting important attributes")
ggsave(roc, filename = '../figs/npdr_relief_auroc_cc.pdf', height = 4, width = 5 )

npdr_relief_auroc_cc <- roc
```
...
pr
ggsave(pr, filename = paste0('../figs/pr_compare_', n.variables, '.pdf'), height = 3, width = 5)
pr_compare_cc <- pr

save(pr_compare_cc, npdr_relief_auroc_cc, npdr_stir_p_cc, npdr_relief_cc, 
     file = '../results/plots_cc.Rdata')
...
save(pr_compare_cc, npdr_stir, pr_df_cc, ss_df_cc, test_df_cc, prr_dat_cc,
