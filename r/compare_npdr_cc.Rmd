...
r2_cc <- round(cor(test.df$stir.log10, test.df$npdr.log10), digits = 3)
...
p <- ggplot(test.df, aes(x=stir.log10,y=npdr.log10)) + 
  ...
  labs(colour = NULL, 
       ...
       title = NULL) + 
  annotate("text", x = 130, y = 130, label = deparse(bquote(R^2 == .(r2_cc))), parse = T) +
...
npdr.beta.cutoff.cc <- glm.stir.cc.results[last.imp.att, 'beta.Z.att']
...
p <- ggplot(test.df, aes(x = rrelief, y = beta.Z.att)) + 
  ...
  annotate("text", x = 0.06, y = 24, label = deparse(bquote(R^2 == .(r2_cc))), parse = T) +
...
```{r}
save(npdr_relief_auroc_cc, npdr_stir_p_cc, npdr_relief_cc,
     npdr_relief_p_cc, npdr_relief_q_cc,
